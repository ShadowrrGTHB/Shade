...
local prompt = useStudio and require(script.Parent.prompt) or loadstring(game:HttpGet('https://raw.githubusercontent.com/ShadowrrGTHB/Night/refs/heads/main/prompt.lua'))()
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(ShadeFolder) then
				if isfile and isfile(ShadeFolder..'/settings'..ConfigurationExtension) then
					file = readfile(ShadeFolder..'/settings'..ConfigurationExtension)
				end
			end
	...
	if not success then 
		if writefile then
			warn('Shade had an issue accessing configuration saving capability.')
		end
	end
end
...
repeat
	if Shade:FindFirstChild('Build') and Shade.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Shade | Build Mismatch')
		print('Shade may encounter issues as you are running an incompatible interface version ('.. ((Shade:FindFirstChild('Build') and Shade.Build.Value) or 'No Build') ..').\n\nThis version of Shade is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Shade = Shade, useStudio and script.Parent:FindFirstChild('Shade') or game:GetObjects("rbxassetid://111017136906881")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(Shade)
	Shade.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Shade.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Shade.Name and Interface ~= Shade then
			Interface.Enabled = false
			Interface.Name = "Shade-Old"
		end
	end
elseif not useStudio then
...
if Shade.AbsoluteSize.X < minSize.X and Shade.AbsoluteSize.Y < minSize.Y then
...
local Main = Shade.Main
local MPrompt = Shade:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Shade:FindFirstChild('Drag')
...
Shade.DisplayOrder = 100
LoadingFrame.Version.Text = Release


local Icons = useStudio and require(script.Parent.icons) or loadstring(game:HttpGet('https://raw.githubusercontent.com/ShadowrrGTHB/Shade/refs/heads/main/icons.lua'))()
...
local Notifications = Shade.Notifications

local SelectedTheme = ShadeLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = ShadeLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Shade.Main.BackgroundColor3 = SelectedTheme.Background
	Shade.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Shade.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Shade.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Shade.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Shade.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Shade.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Shade.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Shade.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Shade:GetDescendants()) do
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Shade had an issue decoding the configuration file, please try delete the file and reopen Shade.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(ShadeLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Shade | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--ShadeLibrary:Notify({Title = "Shade Flags", Content = "Shade was unable to find '"..FlagName.. "' in the save file.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(ShadeLibrary.Flags) do
...
function ShadeLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Shade | Not seeing your actions in notifications?')
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(ShadeFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Shade Settings', 0, true)

	if TabList['Shade Settings'] then
		TabList['Shade Settings'].LayoutOrder = 1000
	end

	if Elements['Shade Settings'] then
		Elements['Shade Settings'].LayoutOrder = 1000
	end
...
function ShadeLibrary:CreateWindow(Settings)
	if Shade:FindFirstChild('Loading') then
		if getgenv and not getgenv().ShadeCached then
			Shade.Enabled = true
			Shade.Loading.Visible = true

			task.wait(1.4)
			Shade.Loading.Visible = false
		end
	end

	if getgenv then getgenv().ShadeCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				ShadeLibrary:Notify({Title = 'Build Mismatch', Content = 'Shade may encounter issues as you are running an incompatible interface version ('.. ((Shade:FindFirstChild('Build') and Shade.Build.Value) or 'No Build') ..').\n\nThis version of Shade is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(ShadeFolder) then
		makefolder(ShadeFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Shade"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Shade Interface Suite" then
		LoadingFrame.Version.Text = "Shade UI"
	end
	...
	if not Settings.DisableShadePrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				ShadeLibrary:Notify({
					Title = "Shade Interface",
					Content = "Thank you for using Shade! - By J",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(ShadeFolder.."/Discord Invites") then
			makefolder(ShadeFolder.."/Discord Invites")
		end

		if isfile and not isfile(ShadeFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(ShadeFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Shade RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(ShadeFolder.."/Key System") then
			makefolder(ShadeFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					...
					warn('Check docs.sirius.menu for help with Shade specific development.')
				end
			end
		end
		...
		if isfile and isfile(ShadeFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(ShadeFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(ShadeFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						ShadeLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= ShadeLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					ShadeLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Shade.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					ShadeLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					ShadeLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ShadeLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= ShadeLibrary.Theme.Default then
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						ShadeLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					ShadeLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			ShadeLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			ShadeLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Shade had an issue creating settings.') end
...
function ShadeLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function ShadeLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function ShadeLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Shade Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.ShadeOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function ShadeLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				ShadeLibrary:Notify({Title = "Shade Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			ShadeLibrary:Notify({Title = "Shade Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Shade Configurations Error | '..tostring(result))
			ShadeLibrary:Notify({Title = "Shade Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = ShadeLibrary:CreateWindow({
		Name = "Shade Example Window",
		LoadingTitle = "Shade Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using Shade may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Shade to get the key from
	...
	--ShadeLibrary:Notify({Title = "Shade Interface", Content = "Welcome to Shade. These - are the brand new notification design for Shade, with custom sizing and Shade calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(ShadeLibrary.Theme) do
